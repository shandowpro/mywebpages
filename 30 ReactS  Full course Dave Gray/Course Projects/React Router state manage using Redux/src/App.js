...
// Importing the inner components Routers pages  : 
...
// [must remove incase of using the redux ] importing the {DataProvider } function [the provider function of  all assinged props inside] => to use it to wrap all App function's called components and routers : 
...
// Importing the required class from the [easy-peasy] library to be used to pull some of the inner defined states :
...
function App() {
  ...
  //       const response = await api.get('/posts');import { DataProvider } from './context/DataContext';
  ...
  // ---------------------------------------
    ...
    // 00 -  Define a [setStates] as extreacted setState function  from the imported {useStoreActions} class of [easy-peasy]   : 
      const setPosts = useStoreActions( (actions) => actions.setPosts ) ;     


    // 1 - Define a variable of destructured object of all exported states props  { data,fetchError,isLoading} coming from  the {useAxiosFetch} custom hook ,with assging the baseURl instead of the using the one deinfeid inside  the  [posts.js ]  :
     const { data, fetchError, isLoading } = useAxiosFetch('http://localhost:3500/posts');

    // 2- [First crud operation] : {Fetching data} using custom hook {useAxiosFetch} fetching method (will be used )  =>
      useEffect(() => {
       setPosts(data)
     }, [data , setPosts])
    // -----------------------------------------------
